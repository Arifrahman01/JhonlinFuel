...
class IssueList extends Component
{
    ...
    {
        $permissions = ["view-loader-issue"];
        abort_if(
            Gate::none($permissions),
    ...
    {
        $message = false;
        $tmpDatas = Issue::where("trans_date", $date)
            ->where("warehouse", $warehouse)
            ->get();
    ...
    public function storeData($data)
    {
        ...
        $newPostingNumber =
            ...
            $data[0]->company_code .
            "/" .
            str_pad($number + 1, 6, "0", STR_PAD_LEFT);
        try {
            foreach ($data as $tmp) {
                ...
                $fuelType = Material::where(
                    "material_code",
                    $tmp->material_code
                )->first();
                $slocId = Sloc::where("sloc_code", $tmp->warehouse)->value(
                    "id"
                );
                ...
                if (!$issue) {
                    throw new \Exception(
                        "Issue not found with ID: " . $tmp->id
                    );
                }
                ...
                $cekStok = MaterialStock::where("company_id", $company->id)
                    ->where("plant_id", $location->id)
                    ->where("sloc_id", $slocId)
                    ->first();
                $paramMovement = [
                    ...
                    "qty" => $tmp->qty,
                    "soh_after" =>
                        toNumber($cekStok->qty_soh) - toNumber($tmp->qty),
            ...
            $this->dispatch(
                "success",
                "Data has been posting :" . $newPostingNumber
            );
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    private function cekData($val)
    {
        ...
        $fuelWarehouseExist = Sloc::where(
            "sloc_code",
            $val->warehouse
        )->exists();
        $transTypeInvalid = in_array($val->trans_type, [
            "ISS",
        ]); /* Hanya untuk ISS/issued */
        $fuelmanExist = Fuelman::where("nik", $val->fuelman)->exists();
        $equipmentExist = Equipment::where(
            "equipment_no",
            $val->equipment_no
        )->exists();
        ...
        $activityExist = Activity::where(
            "activity_code",
            $val->activity
        )->exists();
        $fuelTypeExist = Material::where(
            "material_code",
            $val->material_code
        )->exists();
        ...
        if (!$companyAllowed) {
            $message =
                "Anda tidak punya akses Company code " . $val->company_code;
        }
        if (!$companyExists) {
            $message =
                "Company code " .
        ...
        if (!$equipmentExist) {
            $message =
                "Equipment " . $val->equipment_no . " not registered in master";
        }
        if (!$locationExist) {
            $message =
                "Location " . $val->location . " not registered in master";
        }
        if (!$departmentExist) {
            $message =
                "Department " . $val->department . " not registered in master";
        }
        if (!$activityExist) {
            $message =
                "Activity " . $val->activity . " not registered in master";
        }
        if (!$fuelTypeExist) {
            $message =
                "Fuel type " . $val->fuel_type . " not registered in master";
        }
        return $message;
    }
}
